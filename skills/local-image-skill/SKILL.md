...
```bash
npm --prefix __CLAUDE_IMAGE_ROOT__ run cli -- config
npm --prefix __CLAUDE_IMAGE_ROOT__ run cli -- generate "<prompt>"
npm --prefix __CLAUDE_IMAGE_ROOT__ run cli -- edit --image "<path>" --prompt "<prompt>"
```
...
7. Reply with the saved path and mention that `local-image-skill` used `__CLAUDE_IMAGE_ROOT__`.
...
```text
__CLAUDE_IMAGE_ROOT__/output/history.json
```
...
```text
__CLAUDE_IMAGE_ROOT__/output/images
```
...
- Check `__CLAUDE_IMAGE_ROOT__/output/images` and `__CLAUDE_IMAGE_ROOT__/output/history.json` for a newly created output before retrying.
...
- Do not read or print `~/.local-image-console/config.json` directly.

## Error Handling

- Missing local console project: say the installed Claude Image repository path is missing and the local console must be installed first.
